     ...
     public List<Professor> getProfessores() {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
            ...
            while (rs.next()) {
                ...
                professor.setIdade(rs.getString("idade"));
                professor.setDisciplina(rs.getString("disciplina"));
                listaProfessores.add(professor);
            }
            return listaProfessores;
        } catch (SQLException ex) {
            System.out.println("Erro ao obter professores: " + ex.getMessage());
            return null;
        }
    }

    // Método para obter professores por nome e disciplina
    public List<Professor> getProfessoresNome(String nome, String disciplina) {
        String sql = "SELECT * FROM Professor WHERE nome LIKE ? AND disciplina LIKE ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
            ...
            System.out.println("Erro ao obter professores por nome e disciplina: " + ex.getMessage());
            return null;
        }
    }
